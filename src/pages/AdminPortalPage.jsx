...
const AdminPortalPage = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribeApps = onSnapshot(qApps, (snapshot) => {
      ...
      setApplicants(apps);
      if (apps.length > 0 && !selectedApplicant) {
        setSelectedApplicant(apps[0]);
      }
      if (adminView === 'applications') setLoading(false);
    });
    ...
    const unsubscribeNews = onSnapshot(qNews, (snapshot) => {
      ...
      setArticles(news);
      if (adminView === 'news') setLoading(false);
    });
              ...
              <header className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {loading ? (
                    ...
                    <>
                          <div className="p-8 bg-white rounded-3xl shadow-sm border border-outline-variant/10 relative overflow-hidden group">
                        <div className="relative z-10 flex flex-col h-full">
                          <p className="text-secondary font-label text-[10px] font-bold uppercase tracking-[0.2em]">Active Submissions</p>
                          <h2 className="text-5xl font-headline font-extrabold text-primary mt-4 tracking-tight">{stats.total}</h2>
                      ...
                      <div className="p-8 bg-white rounded-3xl shadow-sm border border-outline-variant/10 relative overflow-hidden group">
                        <div className="relative z-10 flex flex-col h-full">
                          ...
                          <h2 className="text-5xl font-headline font-extrabold text-amber-600 mt-4 tracking-tight">{stats.pending}</h2>
                      ...
                      <div className="p-8 bg-primary-container text-on-primary-container rounded-3xl shadow-xl border border-primary/10 relative overflow-hidden group">
                        <div className="relative z-10 flex flex-col h-full">
                          <p className="font-label text-[10px] font-bold uppercase tracking-[0.2em] opacity-70">Efficiency Rate</p>
                          <h2 className="text-5xl font-headline font-extrabold mt-4 tracking-tight">
                            {stats.efficiency}%
                          </h2>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div className="bg-white/5 p-5 rounded-2xl border border-white/5 group/meta">
                              ...
                              <p className="text-xl font-headline font-black mt-1 text-secondary-fixed">
                                {selectedApplicant.createdAt?.toDate().toLocaleDateString('en-GB', { day: '2-digit', month: 'short' }) || 'Recent'}
                              </p>
                            </div>
                        ...
                        <div className="pt-4 flex flex-col gap-4">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <button 
                              ...
                              className={`py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-lg transition-all duration-300 border border-white/20 ${selectedApplicant.status === 'Selected' ? 'bg-emerald-500 text-white border-none' : 'bg-white/10 hover:bg-emerald-500/20 text-on-primary'}`}
                            >
                              Approve
                            </button>
                            <button 
                              onClick={() => handleUpdateStatus(selectedApplicant.id, 'Rejected')}
                              className={`py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-lg transition-all duration-300 border border-white/20 ${selectedApplicant.status === 'Rejected' ? 'bg-rose-500 text-white border-none' : 'bg-white/10 hover:bg-rose-500/20 text-on-primary'}`}
                            >
                              Disregard
                            </button>
                          </div>
                  ...
                  <div key={article.id} className="bg-white rounded-[3rem] overflow-hidden group shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-700 border border-outline-variant/10">
                    <div className="relative h-64 overflow-hidden">
                      ...
                      <div className="absolute bottom-6 left-6 right-6 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                        <p className="text-[10px] font-bold text-white/80 uppercase tracking-widest">Modified {article.createdAt?.toDate().toLocaleDateString()}</p>
                      </div>
                    </div>
